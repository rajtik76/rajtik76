# 👋 Hi, I'm Vladislav Rajtmajer (@rajtik76)

![Laravel 12 Ready](https://img.shields.io/badge/Laravel-12%20Ready-FF2D20?logo=laravel&logoColor=white&style=for-the-badge)
![Open to Work](https://img.shields.io/badge/Open%20to-Remote%20Work-22c55e?style=for-the-badge)
![PHP](https://img.shields.io/badge/PHP-10%2B%20years-777BB4?logo=php&logoColor=white&style=for-the-badge)

---

💻 **Senior PHP / Laravel Developer**  
🚀 Building SaaS platforms, IoT solutions, and robust web applications  
🧠 Programming since 1997  
🌍 Open to remote collaboration

---

## 🚀 Tech Stack

![PHP](https://img.shields.io/badge/PHP-8.x-777BB4?logo=php&logoColor=white)
![Laravel](https://img.shields.io/badge/Laravel-Framework-FF2D20?logo=laravel&logoColor=white)
![Filament](https://img.shields.io/badge/FilamentPHP-Admin%20Panel-F59E0B?logo=laravel&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-Database-4169E1?logo=postgresql&logoColor=white)
![Vue.js](https://img.shields.io/badge/Vue.js-Frontend-4FC08D?logo=vuedotjs&logoColor=white)
![Livewire](https://img.shields.io/badge/Livewire-Fullstack-FB70A9?logo=laravel&logoColor=white)
![TailwindCSS](https://img.shields.io/badge/TailwindCSS-UI-06B6D4?logo=tailwindcss&logoColor=white)
![Pest](https://img.shields.io/badge/Pest-Tested-10B981)
![IoT](https://img.shields.io/badge/IoT-Arduino%20%26%20Sensors-00979D?logo=arduino&logoColor=white)

---

## 🧠 Experience Background

I have been programming since **1997**, starting with low-level and desktop technologies (C/C++, Delphi, Turbo Pascal, FoxPro, Perl).

This strong foundation helps me design efficient and scalable systems today — especially in:

- modern Laravel applications  
- performance-sensitive backends  
- IoT and hardware integrations  

In recent years, I have focused primarily on **PHP, Laravel, PostgreSQL, and IoT solutions (including Arduino-based systems)**.

---

## 🌐 Featured Projects

- 🔗 **[Billify.cz](https://billify.cz)** — commercial invoicing SaaS  
- 🔗 **[Laravel Blog](https://laravel-blog.cz)** — personal Laravel blog

---

## 🤝 Collaboration

I am open to:

- remote opportunities  
- freelance Laravel projects  
- long-term backend roles  
- interesting SaaS ideas  

If you have something exciting in mind, feel free to reach out!

---

## 📫 Contact

- ✉️ Email: rajtik@gmail.com  
- 🐦 Twitter/X: https://twitter.com/VRajtmajer

---

⭐ _Thanks for visiting my profile!_
